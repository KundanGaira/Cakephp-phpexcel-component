...
//        die;
        ...
        $this->_createChartTable($dataArry, $otherParams, $objWorksheet);
        // }
    ...
    public function _createChartTable($chartData, $otherParams, $objWorksheet) {
        ...
        foreach ($chartData as $indx => $chData) {
            // Chart grouping type
            $chartGroup = '';
            if (!empty($otherParams[$indx])) {
                $chartGroupT = json_decode($otherParams[$indx], true);
                $chartGroup = $chartGroupT['cGroup'];
            }
            ...
            $horizonLabl = $horizontalData = $verticalData = $xAxisTickValues = $dataSeriesLabels = $dataSeriesValues = [];

            $horizontalData = explode($this->csvDelimiter, $chData['category']);
            array_shift($horizontalData); // remove rest unwanted data.
            $horzntlLen = count($horizontalData);

            $verticalData = $chData['data'];
            ...
            for ($i = 0; $i <= $horzntlLen; $i++) { //length+1 first cell need to be empty
                if ($i == 0) {
                    ...
                    $ttxt = trim(str_replace('"', '', $horizontalData[$i - 1]));
                    $tmptxt = $this->_translate($ttxt);
                    array_push($horizonLabl, $tmptxt);  // Step1
                    array_push($dataSeriesLabels, new PHPExcel_Chart_DataSeriesValues('String', 'Worksheet!$' . $this->alphabets[$i] . '$1', NULL, 1)); //Map horizontal labels
                }
            }

            $workshettArry[$lastUsedRow] = $horizonLabl; // Place at 0 index always


            $xAxisTickValues = array(
                new PHPExcel_Chart_DataSeriesValues('String', 'Worksheet!$' . $this->alphabets[0] . '$' . ($lastUsedRow + 2) . ':$' . $this->alphabets[0] . '$' . ($lastUsedRow + 2) + ($verticalLength - 1), NULL, $verticalLength), //	Map vertical labels
            );
            ...
            for ($i = 0; $i < $verticalLength; $i++) {
                ...
                array_push($dataSeriesValues, new PHPExcel_Chart_DataSeriesValues('Number', 'Worksheet!$' . $this->alphabets[$i + 1] . '$' . ($lastUsedRow + 2) . ':$' . $this->alphabets[$i + 1] . '$' . ($lastUsedRow + 2) + ($verticalLength), NULL, $verticalLength)); // Map cell data,
            }
            ...
            for ($k = 0; $k < $verticalLength; $k++) {
                array_push($blank_row, '');
            }
            array_push($workshettArry, $blank_row);
            array_push($workshettArry, $blank_row);
            $lastUsedRow = $lastUsedRow +($verticalLength+1);
        }
        $objWorksheet->fromArray($workshettArry);
//        echo'<pre>';
//        print_r($workshettArry);
//        die;
//
//
//
